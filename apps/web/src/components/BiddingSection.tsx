...
const DEFAULT_AUCTION_ITEMS: AuctionItem[] = [
  ...
  {
    id: 'heritage-yamal-2026',
    title: '2026 Spain Match Issue World Cup Home Shirt L. Yamal #10',
    productId: 'shirt-1',
    type: 'MATCH ISSUE',
    tags: ['🛡️ World Cup 2026 Starter', '🇪🇸 La Roja Nameset'],
    currentBid: 4100,
...
export const BiddingSection: React.FC<BiddingSectionProps> = ({ onAddToCart, setCurrentPage, formatPrice }) => {
  ...
  return (
    <div id="heritage-bidding-section" className="w-full space-y-8 max-w-7xl mx-auto px-6 md:px-12 py-6">
      ...
      <section 
        className="relative rounded-3xl overflow-hidden bg-cover bg-center min-h-[280px] md:min-h-[320px] flex items-center shadow-lg transition-all border border-zinc-800"
        style={{
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {DEFAULT_AUCTION_ITEMS.map((item) => (
          <div 
            key={item.id} 
            className="group bg-[#121212] border border-zinc-800 hover:border-zinc-700 rounded-3xl p-5 flex flex-col justify-between transition-all duration-300 shadow-sm hover:shadow-md relative overflow-hidden"
          >
            
            {/* Jersey visual container */}
            <div className="bg-zinc-900 rounded-2xl p-4 h-[240px] flex items-center justify-center relative border border-zinc-800/10 mb-4 group-hover:bg-zinc-900/45 transition-colors">
            ...
            <div className="space-y-3 flex-grow flex flex-col justify-between">
              ...
              <div className="space-y-1.5">
                <h3 className="text-white font-extrabold text-[13px] leading-snug tracking-tight text-left min-h-[40px] group-hover:text-zinc-400 transition-colors">
                  {item.title}
                </h3>
                ...
                <div className="flex items-center justify-between text-left">
                  ...
                  <div className="flex items-center border border-zinc-800 rounded-lg overflow-hidden bg-zinc-900">
                    <button
                      ...
                      className="px-2.5 py-1 text-xs font-black text-zinc-300 hover:bg-zinc-800/80 transition-colors cursor-pointer"
                    >
                      -
                    </button>
                    <span className="px-3 text-xs font-black font-mono text-white">{quantities[item.id] || 1}</span>
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
      ...
      {addedItem && (
        <div className="fixed inset-0 bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-[#121212] rounded-3xl max-w-md w-full border border-zinc-800 overflow-hidden shadow-2xl relative p-8 text-center space-y-6 animate-scaleUp">
            
            {/* Modal Close Button */}
            <button 
              type="button"
              onClick={() => setAddedItem(null)}
              className="absolute top-5 right-5 p-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-100 rounded-full transition-all cursor-pointer"
              aria-label="Close"
            >
              <X size={16} />
            </button>

            <div className="w-16 h-16 bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-full flex items-center justify-center mx-auto shadow-sm">
            ...
            <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800/30 flex items-center gap-4 text-left">
              <div className="w-16 h-16 shrink-0 bg-[#121212] rounded-xl p-1 border border-zinc-800">
                <JerseyRenderer 
              ...
              <div className="space-y-0.5 overflow-hidden">
                <p className="text-xs font-black text-white truncate">{addedItem.title}</p>
                <p className="text-[10px] font-mono text-zinc-400 font-bold">{addedItem.type} • SIZE L (PLAYER SPEC)</p>
                <p className="text-sm font-bold text-white font-mono">{formatCurrency(addedItem.currentBid)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              <button
                type="button"
                onClick={() => setAddedItem(null)}
                className="bg-zinc-900 hover:bg-zinc-800 text-zinc-100 font-bold text-xs uppercase py-3.5 rounded-full transition-all cursor-pointer"
              >
                Keep Browsing
              </button>
              <button
