...
function cloneGallery(cfg: CommunityGalleryConfig): CommunityGalleryConfig {
  return {
    ...cfg,
...
function cleanGallery(draft: CommunityGalleryConfig): CommunityGalleryConfig {
  return cloneGallery({
    title: draft.title.trim() || DEFAULT_COMMUNITY_GALLERY.title,
    membersLabel: draft.membersLabel.trim() || DEFAULT_COMMUNITY_GALLERY.membersLabel,
    subtitle: draft.subtitle.trim() || DEFAULT_COMMUNITY_GALLERY.subtitle,
    facebookUrl: draft.facebookUrl.trim() || VANSKAP_COMMUNITY_FACEBOOK_URL,
...
export const CommunityGalleryPanel: React.FC<CommunityGalleryPanelProps> = ({
  appConfig,
  onUpdateConfig,
  onRequireStaffLogin,
}) => {
  const [draft, setDraft] = useState<CommunityGalleryConfig>(() =>
    cloneGallery(normalizeCommunityGallery(appConfig.communityGallery)),
  );
  ...
  useEffect(() => {
    if (dirty) return;
    setDraft(cloneGallery(normalizeCommunityGallery(appConfig.communityGallery)));
  }, [appConfig.communityGallery, dirty]);

  const patchDraft = (next: CommunityGalleryConfig) => {
    setDraft(next);
    setDirty(true);
  };

  const updateImage = (id: string, patch: Partial<CommunityGalleryImage>) => {
    const next = {
      ...draftRef.current,
  ...
  const removeImage = async (id: string) => {
    const ok = await confirmAsync('Remove this community image?');
    if (!ok) return;
    patchDraft({
      ...draft,
      images: draft.images
    ...
    patchDraft({
      ...
      images: [
        ...draft.images,
        {
          id,
          imageUrl: '',
          title: `Community ${draft.images.length + 1}`,
          status: 'Active',
  ...
  const resetDefaults = async () => {
    const ok = await confirmAsync('Reset community gallery to default seed images?');
    if (!ok) return;
    patchDraft(cloneGallery(DEFAULT_COMMUNITY_GALLERY));
  };

  const persistGallery = async (gallery: CommunityGalleryConfig, silent = false) => {
    ...
    if (isApiEnabled()) {
      ...
      const saved = await api.updateCommunityGallery(cleaned);
      const published = normalizeCommunityGallery(
        (saved as { communityGallery?: unknown })?.communityGallery ?? cleaned,
      );
      onUpdateConfig((prev) => ({
        ...prev,
        communityGallery: published,
      }));
      setDraft(published);
      setDirty(false);
      if (!silent) toast('Community section published to homepage', 'success');
      return true;
    }

    onUpdateConfig((prev) => ({
      ...prev,
      communityGallery: cleaned,
    }));
    setDraft(cleaned);
    setDirty(false);
    if (!silent) toast('Community section saved', 'success');
    return true;
  };
  ...
  const uploadFor = async (id: string, file: File) => {
    ...
    try {
      ...
      toast('Image live on homepage', 'success');
    } catch (err) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      toast(err instanceof Error ? err.message : 'Failed to save community gallery', 'error');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[12px] text-zinc-600 mt-1 max-w-xl font-medium">
            Manage homepage “Join the VANSKAP Community” — upload each carousel image separately.
            Changes publish to the storefront after Premier League.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <label className="flex items-center gap-3 md:col-span-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3 cursor-pointer">
          ...
          <span className="text-xs font-bold text-zinc-950">
            Show on homepage (after Premier League)
          </span>
        </label>
      ...
      <div className="flex items-center justify-between gap-2 border-t border-zinc-200 pt-4">
        <div>
          ...
          <p className="text-[11px] text-zinc-600 mt-0.5">
            Each slot is one homepage photo — upload or replace individually.
          </p>
        </div>
        <button
          type="button"
          ...
          >
            <div className="relative aspect-[3/4] bg-zinc-100">
              {img.imageUrl ? (
                <img
                  src={img.imageUrl}
                  alt={img.title || `Community image ${index + 1}`}
