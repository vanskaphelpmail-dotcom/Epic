...
type JourneyGalleryConfig = CommunityGalleryConfig;
type JourneyGalleryImage = CommunityGalleryImage;

interface JourneyGalleryPanelProps {
  appConfig: AppConfig;
  onUpdateConfig: (next: AppConfig | ((prev: AppConfig) => AppConfig)) => void;
  onRequireStaffLogin?: () => void;
}

function cloneGallery(cfg: JourneyGalleryConfig): JourneyGalleryConfig {
  return {
    ...cfg,
...
function cleanGallery(draft: JourneyGalleryConfig): JourneyGalleryConfig {
  return cloneGallery({
    title: draft.title.trim() || DEFAULT_JOURNEY_GALLERY.title,
    membersLabel: draft.membersLabel.trim() || DEFAULT_JOURNEY_GALLERY.membersLabel,
    subtitle: draft.subtitle.trim() || DEFAULT_JOURNEY_GALLERY.subtitle,
...
export const JourneyGalleryPanel: React.FC<JourneyGalleryPanelProps> = ({
  appConfig,
  onUpdateConfig,
  onRequireStaffLogin,
}) => {
  const [draft, setDraft] = useState<JourneyGalleryConfig>(() =>
    cloneGallery(normalizeJourneyGallery(appConfig.journeyGallery)),
  );
  ...
  useEffect(() => {
    if (dirty) return;
    setDraft(cloneGallery(normalizeJourneyGallery(appConfig.journeyGallery)));
  }, [appConfig.journeyGallery, dirty]);

  const patchDraft = (next: JourneyGalleryConfig) => {
    setDraft(next);
    setDirty(true);
  };

  const updateImage = (id: string, patch: Partial<JourneyGalleryImage>) => {
    const next = {
      ...draftRef.current,
  ...
  const removeImage = async (id: string) => {
    const ok = await confirmAsync('Remove this journey image?');
    if (!ok) return;
    patchDraft({
      ...draft,
      images: draft.images
    ...
    patchDraft({
      ...
      images: [
        ...draft.images,
        {
          id,
          imageUrl: '',
          title: `Journey ${draft.images.length + 1}`,
          status: 'Active',
  ...
  const resetDefaults = async () => {
    const ok = await confirmAsync('Reset Journey gallery to empty defaults?');
    if (!ok) return;
    patchDraft(cloneGallery(DEFAULT_JOURNEY_GALLERY));
  };

  const persistGallery = async (gallery: JourneyGalleryConfig, silent = false) => {
    ...
    if (isApiEnabled()) {
      ...
      let saved: { journeyGallery?: unknown };
      try {
        saved = await api.updateJourneyGallery(cleaned);
      } catch (primaryErr) {
        try {
          const viaSettings = await api.updateCmsSettings({
            journeyGallery: cleaned,
          });
          saved = {
            journeyGallery:
              (viaSettings as { journeyGallery?: unknown })?.journeyGallery ?? cleaned,
          };
        } catch {
          throw primaryErr;
        }
      }
      const published = normalizeJourneyGallery(
        (saved as { journeyGallery?: unknown })?.journeyGallery ?? cleaned,
      );
      onUpdateConfig((prev) => ({
        ...prev,
        journeyGallery: published,
      }));
      setDraft(published);
      setDirty(false);
      if (!silent) toast('Journey gallery published to About page', 'success');
      return true;
    }
    ...
    setDraft(cleaned);
    setDirty(false);
    if (!silent) toast('Journey gallery saved', 'success');
    return true;
  };
  ...
  const uploadFor = async (id: string, file: File) => {
    ...
    try {
      ...
      if (ok) toast('Image live on About page', 'success');
    } catch (err) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      toast(err instanceof Error ? err.message : 'Failed to save journey gallery', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-5 sm:p-6 space-y-5 animate-fadeIn">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-black uppercase tracking-wider text-zinc-950 flex items-center gap-2">
            ...
            Journey we make Epic Vanskap
          </h2>
          <p className="text-[12px] text-zinc-600 mt-1 max-w-xl font-medium">
            Upload story photos for the About page carousel — separate from Customers Feedback.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Save size={13} />
            {saving ? 'Saving…' : 'Publish to About page'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <label className="flex items-center gap-3 md:col-span-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3 cursor-pointer">
          ...
          />
          <span className="text-xs font-bold text-zinc-950">Show on About page</span>
        </label>
        ...
        <label className="space-y-1">
          ...
          <input
            ...
            placeholder="Five Friends. One Dream. One Vanskap."
            className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm font-semibold text-zinc-950 focus:outline-none focus:border-zinc-950"
          />
        </label>
        <label className="space-y-1">
      ...
      <div className="flex items-center justify-between gap-2 border-t border-zinc-200 pt-4">
        <div>
          ...
          <p className="text-[11px] text-zinc-600 mt-0.5">
            Each slot is one About-page story photo — upload or replace individually.
          </p>
        </div>
        <button
          type="button"
          ...
          >
            <div className="relative aspect-[3/4] bg-zinc-100">
              {img.imageUrl ? (
                <img
                  src={img.imageUrl}
                  alt={img.title || `Journey image ${index + 1}`}
