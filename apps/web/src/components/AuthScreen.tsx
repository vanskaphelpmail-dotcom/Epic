...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  return (
    <div className="max-w-md mx-auto my-12 bg-white border border-emerald-100 rounded-3xl p-8 shadow-md relative overflow-hidden animate-fadeIn text-emerald-950">
      ...
      <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-4 mb-6 space-y-1 text-center">
        <p className="text-xs font-black uppercase text-emerald-950 font-mono">Private admin portal</p>
      ...
      <div className="text-center space-y-2 mb-8">
        ...
        <h2 className="text-2xl font-black uppercase tracking-tight font-sans text-emerald-950">{titles[mode].h}</h2>
        <p className="text-xs text-emerald-700 font-mono">{titles[mode].p}</p>
      </div>
      ...
      {mode === 'forgot' ? (
        <form onSubmit={handleForgot} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full bg-emerald-800 hover:bg-emerald-900 disabled:opacity-60 text-white font-extrabold text-xs uppercase tracking-widest py-3.5 rounded-xl"
          >
        ...
        <form onSubmit={handleReset} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] text-emerald-700 font-mono block uppercase">Reset Token</label>
            <div className="relative">
              <input
                type="text"
                required
                ...
                className="w-full bg-white border border-emerald-100 rounded-xl py-3 pl-11 pr-4 text-xs text-emerald-950 focus:outline-none focus:border-emerald-500 transition-colors"
              />
              <KeyRound size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-700" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] text-emerald-700 font-mono block uppercase">New Password</label>
            <div className="relative">
              <input
                type="password"
                required
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] text-emerald-700 font-mono block uppercase">Admin Email</label>
            <div className="relative">
              <input
                type="email"
                required
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] text-emerald-700 font-mono block uppercase">Password</label>
            <div className="relative">
              <input
                type="password"
                required
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full bg-emerald-800 hover:bg-emerald-900 disabled:opacity-60 text-white font-extrabold text-xs uppercase tracking-widest py-3.5 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 group cursor-pointer"
          >
      ...
      <div className="mt-8 border-t border-emerald-100 pt-4 text-center space-y-2">
        {mode === 'login' && (
          <button
            type="button"
            onClick={() => {
            ...
            className="block w-full text-xs text-emerald-700 hover:text-emerald-950 font-bold underline"
          >
            Forgot password?
          </button>
        )}
